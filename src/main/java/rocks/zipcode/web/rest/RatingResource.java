...
/**
 * REST controller for managing {@link rocks.zipcode.domain.Rating}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class RatingResource {
    ...
    public ResponseEntity<Rating> partialUpdateRating(
        ...
        Optional<Rating> result = ratingRepository
            ...
            .map(existingRating -> {
                if (rating.getAttribute() != null) {
                    existingRating.setAttribute(rating.getAttribute());
                }
                if (rating.getStarRating() != null) {
                    existingRating.setStarRating(rating.getStarRating());
                }
